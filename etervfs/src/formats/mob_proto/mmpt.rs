...
use crate::formats::mob_proto::CHARACTER_NAME_LENGTH;

// External Uses
use lazy_static::lazy_static;


const MAGIC: [char; 4] = ['M', 'M', 'P', 'T'];


lazy_static!(
    pub static ref MAGIC_VERSION_2_FCC: FourCC = four_cc::from_chars(MAGIC);
);
...
impl Table {
    /*
    pub fn from_bytes_manual(data: Vec<u8>) -> Result<Self> {
        let mut cursor = Cursor::new(data);

        let vnum = cursor.read_u32::<LittleEndian>()?;
        let vnum = cursor.read_u32::<LittleEndian>()?;
